...
export function buildSecretPatterns(): SecretPattern[] {
  return [
    ...
    { id: "anthropic_sk", label: "ANTHROPIC", pattern: /\bsk-ant-(?:api\d{2}-)?[A-Za-z0-9_-]{20,500}\b/g },
    { id: "openai_sk", label: "OPENAI", pattern: /\bsk-(?!ant-)[A-Za-z0-9]{20,}\b/g },
    { id: "aws_secret_key", label: "AWS_SECRET", pattern: /\b(?:aws_secret_access_key|AWS_SECRET_ACCESS_KEY)\s*[=:]\s*['"]?[A-Za-z0-9/+=]{40}['"]?/gi },
    { id: "blekline_workspace_token", label: "BLEKLINE_TOKEN", pattern: /\bblw_(?:live|eval|test)_[A-Za-z0-9_-]{8,}\b/g },
    { id: "pem_private_key", label: "PEM_KEY", pattern: /-----BEGIN (?:RSA |EC |OPENSSH |ENCRYPTED )?PRIVATE KEY-----[\s\S]{0,12000}?-----END (?:RSA |EC |OPENSSH |ENCRYPTED )?PRIVATE KEY-----/g },
